...
def _verifier():
    if not getattr(config, "WAVE_AUTO_VERIFY", True):
        return None
    ...
    return load_verifier(
        path,
        getattr(config, "WAVE_MERCHANT_NAME", ""),
        getattr(config, "WAVE_MERCHANT_PHONE", ""),
        max_age_hours=getattr(config, "PAYMENT_TX_MAX_AGE_HOURS", 2),
        proxy=getattr(config, "WAVE_HTTP_PROXY", None),
        verify_ssl=getattr(config, "WAVE_VERIFY_SSL", True),
    )
