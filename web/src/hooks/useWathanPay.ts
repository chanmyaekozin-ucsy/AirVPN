...
export function useWathanPay() {
  ...
  const pay = useCallback(
    async (options: PayOptions | PayParams): Promise<PayResult> => {
      ...
      const rawAmount = options.amount ?? options.amountKs ?? 0;
