...
export async function POST(req: NextRequest) {
  try {
    ...
    // WathanPay now officially supports signed authData — it is the only
    ...
    sub = rawId ? (rawId.startsWith("wp_") ? rawId : `wp_${rawId}`) : "";
    if (!sub) {
      return Response.json({ error: "authData missing user id." }, { status: 401 });
    }
    ...
    const user = await updateStore((store) => {
      ...
      if (!found) {
        found = {
          ...
          // No password credential — WathanPay users sign in via signed authData only.
          balanceKs: 0,
          wathanpaySub: sub,
